...
function getHomePage(data, image, shortTitle, description) {
  var homePage = data.homepage || false;
  ...
  return {
    ...
    async loadContent() {
      ...
      const posts = filePaths.map((relativePath) => {
        ...
        var shortTitle = getShortTitle(data);
        var homePage = getHomePage(data, image, shortTitle, description);
        return {
          title: data.title,
